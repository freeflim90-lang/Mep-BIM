---
type: qwen-product-draft
project: Model Quality Auditor
task_id: ADDIN-IDEA-20260603-002
status: generated
external_code_only: true
created: 2026-06-03
tags:
  - qwen
  - mqa
  - backend-draft
  - product-development
---

# ADDIN-IDEA-20260603-002 Revit Sheet Issue Packager MVP 초안

선정 아이템: Qwen Coder daily 5-item Revit/Navisworks add-in development pipeline

## 작업 정보

| 항목 | 내용 |
|---|---|
| 산출물 | 모델 이슈를 시트, 뷰, 레벨, 공종별 검토 패키지로 묶는 데이터 계약과 Markdown/CSV 출력 구조 |
| 범위 | Revit 시트/뷰 객체 접근은 인터페이스로만 정의하고, 샘플 JSON 입력에서 검토 패키지를 생성하는 순수 로직을 우선한다. |
| 예상 출력 위치 | `backend/product_candidates/sheet_issue_packager.py` |
| Qwen 실행 | 성공 |
| 모델 | qwen2.5-coder:7b |
| 사유 | - |

## Qwen 초안

외부 개발 모드(`CODE_DEV_EXTERNAL_ONLY=true`)에 따라 코드 개발 본문은 Mac mini에 저장하지 않는다.

초안 본문은 Gmail 발송 대상으로만 처리하며, Gmail 설정이 없으면 본문을 폐기하고 재실행해야 한다.

## 연결

- [[Qwen Draft Index]]
- [[Qwen Development Boundary]]
- [[Revit API Test Gate Index]]
- [[Build Test Index]]

---
type: source-index
generated: true
category: AI Knowledge Base
source_path: "data/knowledge_base/시설유형/일반건물_빌딩_BIM.md"
---

# 일반건물 빌딩 BIM 적용 기준 지식 베이스

## Source

`data/knowledge_base/시설유형/일반건물_빌딩_BIM.md`

Absolute path:

`/Users/choejeong-yeon/LUA BIM LABS/data/knowledge_base/시설유형/일반건물_빌딩_BIM.md`

## Category

- [[MOC - AI Knowledge Base]]

## Summary

- Source: 국토부 BIM 기본지침, LH 가이드, 중소 규모 BIM 적용 실무 - Tags: general-building,mixed-use,commercial-tower,bim,lod,2026 **AI 즉시 답변 패턴 — "일반 사무용 빌딩 BIM에서 무엇을 먼저 해야 하나요?"** ``` 일반 사무용 빌딩 BIM 착수 순서: 1. 프로젝트 BEP 작성: LOD 계획·납품물·CDE 환경 확정

## Keywords

ai, api, autodesk, base, bim, bim.md, cs, data, knowledge, knowledge_base, kpi, mep, navisworks, revit, 간섭, 교육, 납품, 지식, 품질

## Related

- [[공연장_문화집회시설 BIM 적용 기준 지식 베이스]]
- [[공장 제조시설 BIM 적용 기준 지식 베이스]]
- [[공항 BIM 적용 기준 지식 베이스]]
- [[데이터센터 BIM 적용 기준 지식 베이스]]
- [[물류센터 창고시설 BIM 적용 기준 지식 베이스]]
- [[발전소 BIM 적용 기준 지식 베이스]]
- [[병원 의료시설 BIM 적용 기준 지식 베이스]]
- [[사옥_기업본사 BIM 적용 기준 지식 베이스]]

## Home

- [[Global Knowledge Map]]

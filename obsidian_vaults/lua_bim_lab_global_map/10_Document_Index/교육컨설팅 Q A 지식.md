---
type: source-index
generated: true
category: AI Knowledge Base
source_path: "data/knowledge_base/qa/교육컨설팅_QA.md"
---

# 교육컨설팅 Q A 지식

## Source

`data/knowledge_base/qa/교육컨설팅_QA.md`

Absolute path:

`/Users/choejeong-yeon/LUA BIM LABS/data/knowledge_base/qa/교육컨설팅_QA.md`

## Category

- [[MOC - AI Knowledge Base]]

## Summary

텔레그램 질문·자동 수집 응답 저장소 — 기준 가이드라인(교육컨설팅.md)과 분리 운영. - Source: LUA BIM LABS 교육 운영 기준, BIM 교육 실무 - Tags: education,consulting,curriculum,bim,training,qa,2026 **Q: BIM 신규 입문자를 위한 교육 커리큘럼은 어떻게 구성하나요?** A: 단계별 커리큘럼: ① 1주차 기초: BIM 개념, Revit UI, 기본 모델링(벽/바닥/지붕) ② 2~3주차

## Keywords

ai, base, bim, data, knowledge, knowledge_base, mep, navisworks, qa, qa.md, revit, 간섭, 개발, 교육, 납품, 온보딩, 지식, 지원, 커리큘럼, 품질

## Related

- [[CEO Q A 지식]]
- [[CFO Q A 지식]]
- [[COO Q A 지식]]
- [[HR_인재분석관 Q A 지식]]
- [[Navisworks_Addin Q A 지식]]
- [[QA_테스터 Q A 지식]]
- [[Revit_Addin_QA]]
- [[간섭검토 Q A 지식]]

## Home

- [[Global Knowledge Map]]

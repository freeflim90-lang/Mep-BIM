---
type: source-index
generated: true
category: Training Curriculum
source_path: "knowledge/60_public/training_curriculum/public_products/05_PRICING_EVOLUTION_ROADMAP.md"
---

# LUA BIM LABS 교육 상품 가격 진화 로드맵

## Source

`knowledge/60_public/training_curriculum/public_products/05_PRICING_EVOLUTION_ROADMAP.md`

Absolute path:

`/Users/choejeong-yeon/LUA BIM LABS/knowledge/60_public/training_curriculum/public_products/05_PRICING_EVOLUTION_ROADMAP.md`

## Category

- [[MOC - Training Curriculum]]

## Summary

문서번호: LBL-TRN-PUB-005 문서상태: 내부 가격 전략 계획 작성일: 2026-05-29 목표: Starter 구독자 30명 이상, 첫 Personal Tutor 5명 검증 **Phase 1 가격 인상 조건**: 없음. 첫 구독자 확보가 우선. 트리거: Telegram 레슨 아카이브 + 공종별 영상 참조 자료 추가 가격 인상 공지에는 다음을 반드시 포함한다: 1. **무엇이 추가되었는가** — 신규 트랙, 레슨 수, 기능 명시

## Keywords

bim, curriculum, knowledge, pricing_evolution_roadmap.md, public, public_products, training, training_curriculum, 교육, 구독, 납품, 변경, 지원, 커리큘럼, 테스트, 품질

## Related

- [[LUA BIM LABS Coordinator Mentor Curriculum v2]]
- [[LUA BIM LABS Personal Tutor Level-Based Curriculum v2]]
- [[LUA BIM LABS Project Mentor Curriculum v2]]
- [[LUA BIM LABS Starter 90-Day MEP BIM Curriculum v2]]
- [[LUA BIM LABS 교육 상품별 커리큘럼 인덱스]]
- [[LUA BIM LABS 상품별 가격-가치 갭 분석]]
- [[2026-06-04 15분 KST 근거기반 응답 교육 카드 샘플]]
- [[2026-06-04 CEO CSO BF 품질진단 상품화 판단 메모 샘플]]

## Home

- [[Global Knowledge Map]]

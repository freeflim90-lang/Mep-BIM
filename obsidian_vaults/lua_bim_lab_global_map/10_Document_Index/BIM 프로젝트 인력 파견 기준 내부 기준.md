---
type: source-index
generated: true
category: AI Knowledge Base
source_path: "data/knowledge_base/BIM_인력파견_기준.md"
---

# BIM 프로젝트 인력 파견 기준 내부 기준

## Source

`data/knowledge_base/BIM_인력파견_기준.md`

Absolute path:

`/Users/choejeong-yeon/LUA BIM LABS/data/knowledge_base/BIM_인력파견_기준.md`

## Category

- [[MOC - AI Knowledge Base]]

## Summary

<!-- 대외 공개 금지 — 내부 인력 배치 의사결정 전용 --> - Source: info.dec-w.com BIM 취업 리포트, 잡코리아 BIM MEP 채용, AURIC MEP BIM 실태조사 - Tags: bim-staffing,mep-bim,recruitment,dispatch,level,2025-2026 **AI 즉시 답변 패턴 — "MEP BIM 전문가는 어디서 구하나요?"** ```

## Keywords

add-in, ai, base, bim, bim_, data, knowledge, knowledge_base, mep, navisworks, revit, 간섭, 교육, 납품, 리스크, 물량, 변경, 역할, 지원, 품질, 회의

## Related

- [[4D 5D BIM 지식 베이스]]
- [[ACC BIM360 CDE 지식 베이스]]
- [[AI 협업운영체계 지식 베이스]]
- [[AX 시간별 신호모니터링 지식 베이스]]
- [[AX 전략승격리뷰 지식 베이스]]
- [[BEP 수행계획서 템플릿]]
- [[BIM 납품검수 지식 베이스]]
- [[BIM 등급별 투입일 기준표 마스터 스케줄 입력용]]

## Home

- [[Global Knowledge Map]]

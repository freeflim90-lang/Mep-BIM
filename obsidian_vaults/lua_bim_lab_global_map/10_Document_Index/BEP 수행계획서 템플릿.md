---
type: source-index
generated: true
category: AI Knowledge Base
source_path: "data/knowledge_base/BEP_수행계획서.md"
---

# BEP 수행계획서 템플릿

## Source

`data/knowledge_base/BEP_수행계획서.md`

Absolute path:

`/Users/choejeong-yeon/LUA BIM LABS/data/knowledge_base/BEP_수행계획서.md`

## Category

- [[MOC - AI Knowledge Base]]

## Summary

- Source: 국토교통부 건설산업 BIM 시행지침(발주자·설계자편), buildingSMART Korea, ISO 19650 - Tags: bep,eir,bim-execution-plan,exchange-information-requirements,public-project,2026 **AI 즉시 답변 패턴 — "BEP(BIM 실행계획서)에 무엇이 들어가야 하나요?"** ``` 국내 공공 BIM 프로젝트 BEP 필수 항목 (국토부 시행지침 기준):

## Keywords

add-in, ai, autodesk, base, bep_, bim, clash, data, knowledge, knowledge_base, mep, navisworks, revit, 간섭, 권한, 납품, 물량, 변경, 역할, 자산, 조직, 품질, 회의

## Related

- [[4D 5D BIM 지식 베이스]]
- [[ACC BIM360 CDE 지식 베이스]]
- [[AI 협업운영체계 지식 베이스]]
- [[AX 시간별 신호모니터링 지식 베이스]]
- [[AX 전략승격리뷰 지식 베이스]]
- [[BIM 납품검수 지식 베이스]]
- [[BIM 등급별 투입일 기준표 마스터 스케줄 입력용]]
- [[BIM 시방서 지식 베이스]]

## Home

- [[Global Knowledge Map]]

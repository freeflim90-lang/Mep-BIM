---
type: source-index
generated: true
category: AI Knowledge Base
source_path: "data/knowledge_base/BIM_지침서.md"
---

# BIM 지침서 지식 베이스

## Source

`data/knowledge_base/BIM_지침서.md`

Absolute path:

`/Users/choejeong-yeon/LUA BIM LABS/data/knowledge_base/BIM_지침서.md`

## Category

- [[MOC - AI Knowledge Base]]

## Summary

- Source: 국토부 BIM 시행지침, LH BIM 적용지침(2024), 조달청 BIM 기본지침, buildingSMART Korea - Tags: bim-guideline,molit,lh,pps,design-standard,comparison,2026 **AI 즉시 답변 패턴 — "국토부 BIM 지침과 LH 지침이 다른가요?"** ``` 국내 주요 발주처 BIM 지침 비교: - 국토교통부 (상위 지침): 모든 공공 BIM의 기준

## Keywords

ai, autodesk, base, bim, bim_, data, knowledge, knowledge_base, mep, model, revit, 간섭, 견적, 납품, 물량, 변경, 조직, 지식, 품질

## Related

- [[4D 5D BIM 지식 베이스]]
- [[ACC BIM360 CDE 지식 베이스]]
- [[AI 협업운영체계 지식 베이스]]
- [[AX 시간별 신호모니터링 지식 베이스]]
- [[AX 전략승격리뷰 지식 베이스]]
- [[BEP 수행계획서 템플릿]]
- [[BIM 납품검수 지식 베이스]]
- [[BIM 등급별 투입일 기준표 마스터 스케줄 입력용]]

## Home

- [[Global Knowledge Map]]

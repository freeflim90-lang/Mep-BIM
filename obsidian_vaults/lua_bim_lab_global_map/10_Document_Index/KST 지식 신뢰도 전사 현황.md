---
type: source-index
generated: true
category: AI Knowledge Base
source_path: "knowledge/10_agents/90_확장에이전트/KST_신뢰도_현황.md"
---

# KST 지식 신뢰도 전사 현황

## Source

`knowledge/10_agents/90_확장에이전트/KST_신뢰도_현황.md`

Absolute path:

`/Users/choejeong-yeon/LUA BIM LABS/knowledge/10_agents/90_확장에이전트/KST_신뢰도_현황.md`

## Category

- [[MOC - AI Knowledge Base]]

## Summary

> 최종 갱신: 2026-06-05 | LUA BIM LABS 운영 기준 > 관련: 지식큐레이터 · 지식업데이트 · 조율차장 > 해당 파일의 **수치·기준 섹션**은 KST01로 처리한다. > 단, "자동수집" 또는 "auto-enrich" 태그가 붙은 섹션은 KST04로 처리. > 이 파일들의 내용은 고객에게 확정 정보로 제시 금지. > "참고 정보" 또는 "자동수집 기준으로 확인 필요" 명시.

## Keywords

add-in, addin, agents, ai, api, autodesk, base, bim, cs, knowledge, kst_, navisworks, revit, store, 간섭, 개발, 견적, 결제, 교육, 납품, 라이선스, 변경, 보안, 빌드, 스토어, 심사, 자산, 지식, 지원, 커리큘럼

## Related

- [[4D 5D BIM 지식 베이스]]
- [[ACC BIM360 CDE 지식 베이스]]
- [[AI 협업운영체계 지식 베이스]]
- [[AX 시간별 신호모니터링 지식 베이스]]
- [[AX 전략승격리뷰 지식 베이스]]
- [[BEP 수행계획서 템플릿]]
- [[BIM 납품검수 지식 베이스]]
- [[BIM 등급별 투입일 기준표 마스터 스케줄 입력용]]

## Home

- [[Global Knowledge Map]]

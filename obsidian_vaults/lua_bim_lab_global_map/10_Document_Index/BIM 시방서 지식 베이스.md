---
type: source-index
generated: true
category: AI Knowledge Base
source_path: "data/knowledge_base/BIM_시방서.md"
---

# BIM 시방서 지식 베이스

## Source

`data/knowledge_base/BIM_시방서.md`

Absolute path:

`/Users/choejeong-yeon/LUA BIM LABS/data/knowledge_base/BIM_시방서.md`

## Category

- [[MOC - AI Knowledge Base]]

## Summary

- Source: 국가건설기준센터(KCSC), KCS 32 10 10:2024 전기설비공사, 국토부 BIM 시방서 - Tags: bim-specification,kcs,standard-specification,electrical,mechanical,2026 **AI 즉시 답변 패턴 — "BIM 시방서와 일반 시방서가 어떻게 다른가요?"** ``` BIM 시방서 vs 일반(표준) 시방서 차이: - 일반 시방서: KCS(표준시방서) — 재료·공법·품질 기준 (기술적

## Keywords

ai, api, base, bim, bim_, cs, data, knowledge, knowledge_base, mep, navisworks, quality, revit, 간섭, 납품, 물량, 역할, 자산, 지식, 품질, 회의

## Related

- [[4D 5D BIM 지식 베이스]]
- [[ACC BIM360 CDE 지식 베이스]]
- [[AI 협업운영체계 지식 베이스]]
- [[AX 시간별 신호모니터링 지식 베이스]]
- [[AX 전략승격리뷰 지식 베이스]]
- [[BEP 수행계획서 템플릿]]
- [[BIM 납품검수 지식 베이스]]
- [[BIM 등급별 투입일 기준표 마스터 스케줄 입력용]]

## Home

- [[Global Knowledge Map]]

---
type: source-index
generated: true
category: AI Knowledge Base
source_path: "knowledge/10_agents/01_경영진/CFO.md"
---

# CFO 지식 베이스

## Source

`knowledge/10_agents/01_경영진/CFO.md`

Absolute path:

`/Users/choejeong-yeon/LUA BIM LABS/knowledge/10_agents/01_경영진/CFO.md`

## Category

- [[MOC - AI Knowledge Base]]

## Summary

- Source: `knowledge/10_agents/conflict_resolution/PRODUCT_LAUNCH_STATUS_SOURCE_OF_TRUTH_DRAFT_20260605.md` - Tags: cfo,pricing,launch-status,source-of-truth,draft CEO/CFO 최종 승인 전 고객 공개 판매 기준은 Starter만 SALE_READY로 본다. Personal Tutor와 Coordinator Mentor는 COMING

## Keywords

add-in, agents, ai, api, autodesk, base, bim, cfo.md, cs, knowledge, store, 개발, 견적, 결제, 교육, 구독, 라이선스, 리스크, 변경, 스토어, 슬라이드, 심사, 역할, 자산, 조직, 지식, 지원, 품질

## Related

- [[CEO 지식 베이스]]
- [[COO 지식 베이스]]
- [[조율차장 지식 베이스]]
- [[10_agents - conflict_resolution - 10_agents - conflict_resolution - AI 간 판단 충돌 해소 체계]]
- [[4D 5D BIM 지식 베이스]]
- [[ACC BIM360 CDE 지식 베이스]]
- [[AI Collaboration Daily Handoff - 2026-06-06]]
- [[AI 간 판단 충돌 로그]]

## Home

- [[Global Knowledge Map]]

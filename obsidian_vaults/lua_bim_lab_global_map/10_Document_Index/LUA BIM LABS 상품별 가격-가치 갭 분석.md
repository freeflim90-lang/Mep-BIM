---
type: source-index
generated: true
category: Training Curriculum
source_path: "knowledge/60_public/training_curriculum/public_products/06_PRICE_VALUE_GAP_ANALYSIS.md"
---

# LUA BIM LABS 상품별 가격-가치 갭 분석

## Source

`knowledge/60_public/training_curriculum/public_products/06_PRICE_VALUE_GAP_ANALYSIS.md`

Absolute path:

`/Users/choejeong-yeon/LUA BIM LABS/knowledge/60_public/training_curriculum/public_products/06_PRICE_VALUE_GAP_ANALYSIS.md`

## Category

- [[MOC - Training Curriculum]]

## Summary

문서번호: LBL-TRN-PUB-006 문서상태: 상품 디밸롭먼트 전략 작성일: 2026-05-29 $119는 온라인 피트니스 코칭($80~150)과 어학 개인 튜터($100~200) 사이다. 이 가격대 소비자 심리는 **"나만을 위한 선생님이 있다"** 는 개인화 투자다. - 레벨 진단 후 5가지 경로 중 선택 - 개인화 일일 레슨 - 주 3회 Q&A - 약점 패턴 기록 (Obsidian) $490+는 임원 코칭($400~800/월), 전문 컨설팅 리테이너의 

## Keywords

bim, curriculum, knowledge, price_value_gap_analysis.md, public, public_products, qa, training, training_curriculum, 개발, 결제, 교육, 구독, 납품, 자산, 지원, 진단, 커리큘럼, 품질

## Related

- [[LUA BIM LABS Coordinator Mentor Curriculum v2]]
- [[LUA BIM LABS Personal Tutor Level-Based Curriculum v2]]
- [[LUA BIM LABS Project Mentor Curriculum v2]]
- [[LUA BIM LABS Starter 90-Day MEP BIM Curriculum v2]]
- [[LUA BIM LABS 교육 상품 가격 진화 로드맵]]
- [[LUA BIM LABS 교육 상품별 커리큘럼 인덱스]]
- [[2026-06-04 15분 KST 근거기반 응답 교육 카드 샘플]]
- [[2026-06-04 BIM 모델러 BF QA 체크리스트 샘플]]

## Home

- [[Global Knowledge Map]]

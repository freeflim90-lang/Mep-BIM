---
type: source-index
generated: true
category: AI Knowledge Base
source_path: "knowledge/10_agents/06_스토어상용화팀/고객지원CS.md"
---

# 고객지원 CS 지식 베이스

## Source

`knowledge/10_agents/06_스토어상용화팀/고객지원CS.md`

Absolute path:

`/Users/choejeong-yeon/LUA BIM LABS/knowledge/10_agents/06_스토어상용화팀/고객지원CS.md`

## Category

- [[MOC - AI Knowledge Base]]

## Summary

- Source: `data/knowledge_base/conflict_resolution/PRODUCT_LAUNCH_STATUS_SOURCE_OF_TRUTH_DRAFT_20260605.md` - Tags: cs,pricing,launch-status,coming-soon,consultation-only,draft 고객이 교육/멘토링 상품 가격이나 결제 링크를 문의하면 CS는 상품별 Launch Status를 먼저 확인한다. CEO/CFO 승인 전 확정 판매 문

## Keywords

add-in, agents, ai, base, bim, clash, cs, cs.md, knowledge, qa, quality, revit, sow, 개발, 견적, 결제, 교육, 납품, 라이선스, 변경, 보안, 스토어, 심사, 역할, 지식, 지원, 품질, 회의

## Related

- [[라이선스결제 지식 베이스]]
- [[스토어심사 지식 베이스]]
- [[제품패키징 지식 베이스]]
- [[10_agents - conflict_resolution - 10_agents - conflict_resolution - AI 간 판단 충돌 해소 체계]]
- [[4D 5D BIM 지식 베이스]]
- [[ACC BIM360 CDE 지식 베이스]]
- [[AI Collaboration Daily Handoff - 2026-06-06]]
- [[AI 간 판단 충돌 로그]]

## Home

- [[Global Knowledge Map]]

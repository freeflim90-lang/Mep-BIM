...
> 목적: `ESCALATED_REDECISION_EXECUTION_PLAYBOOK_20260606.md`가 2026-06-12 실제 결과 적용 전에 네 가지 결과 분기에서 일관된 상태값을 산출하는지 검증한다. 승인 분기는 원건을 닫을 수 있지만, 자동 감사는 `CLOSED` 세션의 `ESCALATE` consensus를 허용하지 않는다. 따라서 Register 상태를 `CLOSED`로 바꾸면 consensus도 반드시 `CONSENSUS_WITH_GUARDRA

---
type: source-index
generated: true
category: Domain Sources
source_path: "knowledge/50_domain/bim_scripts/openBES-ecosystem/OpenBES-py/dev-wishlist.md"
---

# OpenBES-py Development Wishlist Post-Excel

## Source

`knowledge/50_domain/bim_scripts/openBES-ecosystem/OpenBES-py/dev-wishlist.md`

Absolute path:

`/Users/choejeong-yeon/LUA BIM LABS/knowledge/50_domain/bim_scripts/openBES-ecosystem/OpenBES-py/dev-wishlist.md`

## Category

- [[MOC - Domain Sources]]

## Summary

Remove fields that no longer have meaning, and tighten units/descriptions so the schema stands on its own without the Excel context. The JSON↔TOML conversion layer bakes in Excel-era defaults such as fixed zone names and index positions, which makes it harder 

## Keywords

bim, bim_scripts, cs, dev-wishlist.md, domain, knowledge, openbes-ecosystem, openbes-py, sources

## Related

- [[Agent Context for OpenBES-py Thermal Simulation Refactoring]]
- [[openBES-ecosystem - OpenBES-py - openBES-ecosystem - OpenBES-py - OpenBES-py]]
- [[OpenBES-py Shared Specification Vulnerability Report]]
- [[11034 - A2__Rules - 11034 - A2__Rules - A2 Rules]]
- [[11034-advanced-bim - Assignment 3 - 11034-advanced-bim - Assignment 3 - Assignment 3]]
- [[7-Day GitHub Proof Plan]]
- [[AI-POWERED-ENERGY-CONSUMPTION-FORECASTING - data - AI-POWERED-ENERGY-CONSUMPTION-FORECASTING - data - Data Fol]]
- [[AI-POWERED-ENERGY-CONSUMPTION-FORECASTING - docs - AI-POWERED-ENERGY-CONSUMPTION-FORECASTING - docs - Document]]

## Home

- [[Global Knowledge Map]]

---
type: source-index
generated: true
category: AI Knowledge Base
source_path: "knowledge/10_agents/90_확장에이전트/BIMobject_패밀리_인덱스.md"
---

# BIMobject 패밀리 라이브러리 인덱스

## Source

`knowledge/10_agents/90_확장에이전트/BIMobject_패밀리_인덱스.md`

Absolute path:

`/Users/choejeong-yeon/LUA BIM LABS/knowledge/10_agents/90_확장에이전트/BIMobject_패밀리_인덱스.md`

## Category

- [[MOC - AI Knowledge Base]]

## Summary

> 2026-06-05 현황 확인: 총 114,506개 패밀리 (6월 1일 기준, 최신 자동 수집값) > KST04 자동수집 — 링크 유효성은 사용 전 확인 필요. **AI 즉시 답변 — "BIMobject에서 MEP 패밀리를 어떻게 찾나요?"** ``` BIMobject.com 검색 방법: 1. 직접 검색: bimobject.com → 카테고리별 검색 2. 이 인덱스에서 공종별 섹션 확인: - 공조(HVAC): 6,958개 / 냉동 및 냉각: 1,200개+

## Keywords

agents, ai, base, bim, bimobject_, cs, knowledge, mep, model, revit, 보안

## Related

- [[4D 5D BIM 지식 베이스]]
- [[ACC BIM360 CDE 지식 베이스]]
- [[AI 협업운영체계 지식 베이스]]
- [[AX 시간별 신호모니터링 지식 베이스]]
- [[AX 전략승격리뷰 지식 베이스]]
- [[BIM 시방서 지식 베이스]]
- [[BIM 시장 규모 분석 지식 베이스]]
- [[BIM 지침서 지식 베이스]]

## Home

- [[Global Knowledge Map]]

---
type: source-index
generated: true
category: AI Knowledge Base
source_path: "data/knowledge_base/AX_전략승격리뷰.md"
---

# AX 전략승격리뷰 지식 베이스

## Source

`data/knowledge_base/AX_전략승격리뷰.md`

Absolute path:

`/Users/choejeong-yeon/LUA BIM LABS/data/knowledge_base/AX_전략승격리뷰.md`

## Category

- [[MOC - AI Knowledge Base]]

## Summary

- Source: 중소벤처기업부 AX 원스톱 바우처, 정책브리핑(M.AX 얼라이언스), DEC-W BIM 뉴스 - Tags: ax,ai-transformation,government-support,construction-ai,bim-ax,2026 **AI 즉시 답변 패턴 — "2026년 한국 AX 지원사업이 있나요?"** ``` 2026년 주요 AX 지원사업: 1. AX 원스톱 바우처: AI 개발·클라우드(GPU·SaaS)·데이터 구매 통합 지원

## Keywords

add-in, ai, api, audit, autodesk, ax_, base, bim, cs, data, knowledge, knowledge_base, mep, model, okr, qa, quality, revit, store, 간섭, 개발, 견적, 교육, 납품, 라이선스, 변경, 빌드, 지식, 지원, 커리큘럼

## Related

- [[4D 5D BIM 지식 베이스]]
- [[ACC BIM360 CDE 지식 베이스]]
- [[AI 협업운영체계 지식 베이스]]
- [[AX 시간별 신호모니터링 지식 베이스]]
- [[BEP 수행계획서 템플릿]]
- [[BIM 납품검수 지식 베이스]]
- [[BIM 등급별 투입일 기준표 마스터 스케줄 입력용]]
- [[BIM 시방서 지식 베이스]]

## Home

- [[Global Knowledge Map]]

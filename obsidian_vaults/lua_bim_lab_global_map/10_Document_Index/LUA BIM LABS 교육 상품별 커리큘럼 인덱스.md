---
type: source-index
generated: true
category: Training Curriculum
source_path: "docs/training_curriculum/public_products/00_PRODUCT_CURRICULUM_INDEX.md"
---

# LUA BIM LABS 교육 상품별 커리큘럼 인덱스

## Source

`docs/training_curriculum/public_products/00_PRODUCT_CURRICULUM_INDEX.md`

Absolute path:

`/Users/choejeong-yeon/LUA BIM LABS/docs/training_curriculum/public_products/00_PRODUCT_CURRICULUM_INDEX.md`

## Category

- [[MOC - Training Curriculum]]

## Summary

문서번호: LBL-TRN-PUB-000 문서상태: 상품 커리큘럼 마스터 플랜 작성일: 2026-05-29 배포등급: Internal Planning - 내부 MEP BIM 커리큘럼: `../mep_bim_ai/00_CURRICULUM_INDEX.md` - 레슨 품질 기준: `../../starter_plan_lesson_quality_standard.md` - 상품 포지셔닝: `../../personal_mep_bim_tutor_positioning.md`

## Keywords

api, bim, cs, curriculum, docs, mep, product_curriculum_index.md, public_products, qa, quality, revit, training, training_curriculum, 간섭, 개발, 교육, 커리큘럼, 품질

## Related

- [[LUA BIM LABS Coordinator Mentor Curriculum v2]]
- [[LUA BIM LABS Personal Tutor Level-Based Curriculum v2]]
- [[LUA BIM LABS Project Mentor Curriculum v2]]
- [[LUA BIM LABS Starter 90-Day MEP BIM Curriculum v2]]
- [[LUA BIM LABS 교육 상품 가격 진화 로드맵]]
- [[LUA BIM LABS 상품별 가격-가치 갭 분석]]
- [[2026-06-04 15분 KST 근거기반 응답 교육 카드 샘플]]
- [[2026-06-04 BIM 모델러 BF QA 체크리스트 샘플]]

## Home

- [[Global Knowledge Map]]

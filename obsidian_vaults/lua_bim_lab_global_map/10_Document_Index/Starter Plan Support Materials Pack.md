---
type: source-index
generated: true
category: Training Curriculum
source_path: "knowledge/60_public/training_curriculum/public_products/launch_packages/starter/STARTER_SUPPORT_MATERIALS.md"
---

# Starter Plan Support Materials Pack

## Source

`knowledge/60_public/training_curriculum/public_products/launch_packages/starter/STARTER_SUPPORT_MATERIALS.md`

Absolute path:

`/Users/choejeong-yeon/LUA BIM LABS/knowledge/60_public/training_curriculum/public_products/launch_packages/starter/STARTER_SUPPORT_MATERIALS.md`

## Category

- [[MOC - Training Curriculum]]

## Summary

> LUA BIM LABS Starter Plan | USD 39/month | 90-Day MEP BIM Education via Telegram > Last updated: 2026-05-29 Send on: Day 7 **Project File Setup** - [ ] Use the correct project template (MEP discipline-specific) - [ ] Confirm units: millimeters (metric) or fe

## Keywords

bim, clash, curriculum, knowledge, launch_packages, mep, model, public, public_products, quality, revit, starter, starter_support_materials.md, training, training_curriculum

## Related

- [[LUA BIM LABS Starter Plan Delivery SOP]]
- [[2026-06-04 CS BF 근거기반 고객 응답 샘플]]
- [[2026-06-04 개발 R D BF 자동검수 티켓 샘플]]
- [[2026-06-04 개발 R D 로그 마스킹 보안 티켓 샘플]]
- [[BIM Execution Plan BEP Template]]
- [[BIM Model QA QC Checklists HVAC and Electrical]]
- [[Coordinator Mentor Case Studies]]
- [[Coordinator Mentor Delivery SOP]]

## Home

- [[Global Knowledge Map]]

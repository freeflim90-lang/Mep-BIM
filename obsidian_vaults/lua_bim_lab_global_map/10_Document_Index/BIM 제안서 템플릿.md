---
type: source-index
generated: true
category: AI Knowledge Base
source_path: "data/knowledge_base/BIM_제안서.md"
---

# BIM 제안서 템플릿

## Source

`data/knowledge_base/BIM_제안서.md`

Absolute path:

`/Users/choejeong-yeon/LUA BIM LABS/data/knowledge_base/BIM_제안서.md`

## Category

- [[MOC - AI Knowledge Base]]

## Summary

- Source: 국토부 건설산업 BIM 시행지침, 발주처별 BIM 지침, 한국 BIM 제안서 실무 - Tags: bim-proposal,rfp,public-project,scope,pricing,2026 **AI 즉시 답변 패턴 — "BIM 용역 제안서에 무엇을 포함해야 하나요?"** ``` BIM 용역 제안서 필수 구성 (공공·민간 공통): 1. 회사 소개: BIM 실적·전문인력·자격 (BIM 전문가 수·경력)

## Keywords

add-in, ai, autodesk, base, bim, bim_, clash, data, knowledge, knowledge_base, mep, navisworks, revit, 간섭, 교육, 권한, 납품, 물량, 변경, 역할, 조직, 지원, 품질, 회의

## Related

- [[4D 5D BIM 지식 베이스]]
- [[ACC BIM360 CDE 지식 베이스]]
- [[AI 협업운영체계 지식 베이스]]
- [[AX 시간별 신호모니터링 지식 베이스]]
- [[AX 전략승격리뷰 지식 베이스]]
- [[BEP 수행계획서 템플릿]]
- [[BIM 납품검수 지식 베이스]]
- [[BIM 등급별 투입일 기준표 마스터 스케줄 입력용]]

## Home

- [[Global Knowledge Map]]

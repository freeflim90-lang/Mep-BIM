...
def ifc_proc(ifc_loc):
    ...
    valve = []
    for v in ifc.by_type('IfcValve'):
        placement = ifc_t.add(v.ObjectPlacement)
        shape = ifc_t.add(v.Representation)
        vv = ifc_t.create_entity('IfcValve', GlobalId=v.GlobalId, Name=v.Name,
                            Representation=shape,
                            ObjectPlacement=placement
                            )
        valve.append(vv)
    for v in ifc.by_type('IfcPipeSegment'):
        placement = ifc_t.add(v.ObjectPlacement)
        shape = ifc_t.add(v.Representation)
        vv = ifc_t.create_entity('IfcPipeSegment', GlobalId=v.GlobalId, Name=v.Name,
                            Representation=shape,
                            ObjectPlacement=placement
                            )
        valve.append(vv)
    ...
    ifc_t.create_entity('IfcRelAggregates',
                        ifcopenshell.guid.new(), owner_t, None, None, site_t, valve)
...
for file in os.listdir(script_path):
    if file.endswith('.ifc') and not file.endswith('_clean.ifc'):
        ifc_file.append(os.path.join(script_path, file))

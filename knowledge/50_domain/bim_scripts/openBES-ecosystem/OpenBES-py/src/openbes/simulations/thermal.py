...
def _calculate_temperatures(
    ...
    building_thermal_mass_t = (
        prev_thermal_mass * (internal_heat_capacity_w - 0.5 * (htr_3 + Htr_em)) + m_tot
...
class ThermalSimulation(HourlySimulation):
    ...
    def _populate_cache(self) -> dict:
        ...
        static = {
            ...
            "internal_heat_capacity_w": self.internal_heat_capacity
            / 3600,  # J/K to W/K [Hourly simulation AM93]
            "mass_factor_scaled": (
            ...
            "ventilation_air_supply_rate": array(
                self.ventilation.air_supply_rate.values
            ),
    ...
    def _calculate_hour_row(self, i: int, prev_row: dict = None) -> dict:
        ...
        night_ventilation_enabled = (
            ...
            (
                (6 <= month < 9)
                or (month == 9 and day == 1 and self._cache["hour"][i] == 1)
            )
        ...
        m = self._cache["mass_factor_scaled"] * (
            0.5 * internal_heat
            + ((solar_heat_windows + solar_heat_opaque) / conditioned_floor_area)
        )

        """
        12. temp_st [Hourly Simulation column AS]
        """
        temp_st = self.theta_st_partial * (0.5 * internal_heat + solar_heat)
        ...
        def calculate_temperatures(
            ...
            return _calculate_temperatures(
                ...
                Htr_is=Htr_is,
                Htr_ms=9.1
                * self.geometry.heat_capacity_am,  # [Hardcoded in Hourly Simulation cell AR95]
                Htr_w=self.geometry.heat_transfer_rate_windows,
                internal_heat_capacity_w=internal_heat_capacity_w,
            )

        m_tot, building_thermal_mass_t, internal_surface_temp, air_free_temp = (
            calculate_temperatures(hc_need=Hc_nd, prev_tm=prev_thermal_mass)
        )
        building_thermal_mass = (prev_thermal_mass + building_thermal_mass_t) / 2
        ...
        prev_thermal_mass_hc_actual = get_prev(
            "building_thermal_mass_hc_actual_t", starting_thermal_mass
        )
        _, _, _, air_free_temp_hc_0 = calculate_temperatures(
            hc_need=0.0, prev_tm=prev_thermal_mass_hc_actual
        )
        ...
        _, _, _, air_free_temp_hc_10 = calculate_temperatures(
            hc_need=10.0, prev_tm=prev_thermal_mass_hc_actual
        )
        ...
        if needs_heating or needs_cooling:
            heating_cooling_demand = (
                10
                * (air_set_temp - air_free_temp_hc_0)
                / (air_free_temp_hc_10 - air_free_temp_hc_0)
            )
        else:
            heating_cooling_demand = 0.0
        _, building_thermal_mass_hc_actual_t, _, air_free_temp_hc_actual = (
            calculate_temperatures(
                hc_need=heating_cooling_demand, prev_tm=prev_thermal_mass_hc_actual
            )
        )

        building_thermal_mass_hc_actual = (
            prev_thermal_mass_hc_actual + building_thermal_mass_hc_actual_t
        ) / 2
    ...
    def set_point_temperature(self) -> DataFrame:
        ...
        """
        if (
            "min_temp_set_point" not in self._hours.columns
            or "max_temp_set_point" not in self._hours.columns
        ):
            self._hours["max_temp_set_point"] = (
                self._hours["is_daytime"] * self.spec.setpoint_winter_day
                + logical_not(self._hours["is_daytime"])
                * self.spec.setpoint_winter_night
            )
            self._hours["min_temp_set_point"] = (
                self._hours["is_daytime"] * self.spec.setpoint_summer_day
                + logical_not(self._hours["is_daytime"])
                * self.spec.setpoint_summer_night
            )
    ...
    def get_heating_and_cooling_degrees_days(
        ...
        days = (
            ...
            .reset_index(["month", "day"])
        )
        days["day"] = days.index + 1
        days = days.set_index(["day"])
        days["heating_degree_day"] = days["temp_air"].apply(
            lambda x: max(0, base_temperature - x)
        )
        days["cooling_degree_day"] = days["temp_air"].apply(
            lambda x: max(0, x - base_temperature)
        )
    ...
    def internal_heat_from_lighting(self) -> "Series[float]":
        ...
        if "internal_heat_from_lighting" not in self._hours.columns:
            ...
            self._hours["internal_heat_from_lighting"] = (ratio * heat) + parasitic
    ...
    def internal_heat_adjusted(self) -> "Series[float]":
        ...
        if "internal_heat_adjusted" not in self._hours.columns:
            ...
            self._hours["internal_heat_adjusted"] = (
                self.internal_heat * adjustment_factor
            )
    ...
    def solar_radiation_glazing_adjustment(self) -> "Series[float]":
        ...
        if self._solar_radiation is None:
            ...
            for point in COMPASS_POINTS:
                ...
                diffuse = (
                    gl_value
                    * hemispheric_integration
                    * self.solar_irradiation.get_diffuse_component(point)
                )
                ground = (
                    gl_value
                    * hemispheric_integration
                    * self.solar_irradiation.ground_reflected_component
                )
                numerator = beam + diffuse + ground
                self._solar_radiation[point] = (
                    numerator / self.solar_irradiation.solar_irradiation[point]
                )
    ...
    def _solar_heat_windows(self) -> "Series[float]":
        ...
        """
        compass_points = sorted(
            [self.geometry.get_compass_point_for_orientation(o) for o in ORIENTATIONS]
        )
        window_mask = self.geometry.window_areas.index.get_level_values(
            "compass_point"
        ).isin(compass_points)
        window_areas = (
            self.geometry.window_areas[window_mask].groupby("compass_point").sum()
        )
        adjusted_area = (1 - self.spec.window_frame_factor) * window_areas
        area_summer = (
            adjusted_area
            * self.spec.solar_external_shading_summer
            * self.spec.parameters.shading_correction_factor
        )
        area_winter = (
            adjusted_area
            * self.spec.solar_external_shading_winter
            * self.spec.parameters.shading_correction_factor
        )
        ...
        Fsh_ob_total = (
            Fsh_ob_overhand * Fsh_ob_fin * Fsh_ob_horizon - solar_lost_through_windows
        )
        ...
        reduction = DataFrame(
            outer(self.rse, window_areas),
            columns=window_areas.index,
        ...
        return {
            "summer": maximum(incoming_radiation_summer - reduction, 0.0),
            "winter": maximum(incoming_radiation_winter - reduction, 0.0),
        }
    ...
    def get_solar_heat_opaque(self, row: Series) -> "Series[float]":
        ...
        Fsh_ob_own = (self.spec.building_length + self.spec.building_width) / (
            self.geometry.equivalent_rectangle.length
            + self.geometry.equivalent_rectangle.width
        )
        ...
        return maximum(
            Fsh_ob * (absorption * Rse * opaque_area * u_value) * solar_radiation
    ...
    def solar_heat_opaque(self) -> "Series[float]":
        ...
        if "solar_heat_opaque" not in self._hours.columns:
            ...
            self._hours["solar_heat_opaque"] = (
                opaque_facade_by_orientation.sum() + self.solar_heat_roof
            )

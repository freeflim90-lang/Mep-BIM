...
logo_tekla = "https://vectorseek.com/wp-content/uploads/2023/12/Tekla-Icon-Logo-Vector.svg-.png"
...
st.markdown("## Isolation Forest-based Anomaly Detection for Structural Steel Elements")
st.markdown(
    body="This application analyzes IFC models to detect potential anomalies in structural steel. "
         "Using the Isolation Forest unsupervised machine learning model, it highlights possible outliers for selected element types (such as plates, beams, or columns).",
    width=1200
)
...
element_type = st.sidebar.selectbox(
    label="**II.** Choose one element type to analyse",
...
if "df_result" in st.session_state and st.session_state["data_loaded"]:
    ...
    # 4.3 SUMMARY STATISTICS: Display some key metrics
    total_count = len(df_result)
    anomaly_count = (df_result["Anomaly Flag"] == -1).sum()
    normal_count = (df_result["Anomaly Flag"] == 1).sum()

    st.subheader("Summary statistics")
    # st.write(f"Total elements analysed: **{total_count}**")
    # st.write(f"Normal elements (Anomaly Flag = 1): **{normal_count}**")
    # st.write(f"Anomalous elements (Anomaly Flag = -1): **{anomaly_count}**")

    col_text, col_num = st.columns(
        spec=[2.5, 1],
        gap="small",
        width=400
        )
    ...
    with col_text:
        st.write("Total elements analysed:")
        st.write(f"Normal elements (Anomaly Flag = 1):")
        st.write(f"Anomalous elements (Anomaly Flag = -1):")

    with col_num:
        st.write(f"**{total_count}**")
        st.write(f"**{normal_count}**")
        st.write(f"**{anomaly_count}**")
    ...
    plot_columns = [column for column in numerical_columns if column not in ["Anomaly Flag"]]
    ...
    with col1:
        st.info(
            ...
            **Interpretation Guide:**\n
             **Score Distribution (Left):** Visualizes the model's decision boundary. Elements with a score **below 0 (red)** are classified as anomalies, while those in the **positive range (blue)** are considered normal.\n
             **Feature Scatter Plot (Right):** Helps identify physical drivers of anomalies. Use the **dropdown menus** above to explore correlations between geometric attributes (e.g., *Length vs. Weight*) and the detected outliers.
            """
        )

    # DROPDOWN for scatterplot X and Y axis
    with col2:
        # Default X-axis parameter is the score and i'm going to compare it to the selected parameter from selectbox
        default_x = "Anomaly Score" # but it can be changed, too, to any other option
        x_axis = st.selectbox(
        ...
        # Default Y-axis: something that is different from the X-axis
        default_y = "Weight (kg)" if "Weight (kg)" in plot_columns else (plot_columns[1] if len(plot_columns) > 1 else plot_columns[0])
        y_axis = st.selectbox(
    ...
    if not df_result.empty:
        fig = plot_anomaly_visualization(df_result, x_axis, y_axis, element_type)
        st.pyplot(fig)
    else:
    ...
    elif filter_option == "Only anomalies":
        df_filtered = df_filtered[df_filtered["Anomaly Flag"] == -1]
    elif filter_option == "Only normal":
        df_filtered = df_filtered[df_filtered["Anomaly Flag"] == 1]
    else:
        pass

    if "Anomaly Score" in df_filtered.columns:
    ...
    # 4.7 Export filtered results to CSV

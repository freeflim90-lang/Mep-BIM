...
def plot_anomaly_visualization(df: pandas.DataFrame,
                               x_axis_col: str,
                               y_axis_col: str,
                               element_type: str) -> matplotlib.pyplot.Figure:
    """
    .. note::
    ...
    :param df:
        The input (passed result) DataFrame which contains the **, *Anomaly Flag* and feature columns.
    :param x_axis_col:
    ...
    scores_series = df["Anomaly Score"].dropna()
    ...
    # axes[0].axvline(x=0, color="black", linestyle="--", linewidth=2) # this is a line of decision boundary
    axes[0].set_title(f"Anomaly Score Distribution for {element_type}s\n")
    ...
    seaborn.scatterplot(
        ...
        x=x_axis_col,
        y=y_axis_col,
        hue="Anomaly Flag", # color by label, which means that the points will be colored according to their label
        style="Anomaly Flag",
        palette={1: "skyblue", -1: "red"},
        markers={1: "o", -1: "s"},
        s=100, # point size
    ...
    axes[1].set_title(f"Anomaly Map for {element_type}s\n{x_axis_col} vs. {y_axis_col}")

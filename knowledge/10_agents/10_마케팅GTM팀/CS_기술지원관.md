...
## 기술지원 CS 운영 기준 업데이트 (2026-06-19)
- Source: auto-enrich via Naver+Tavily+Google+DDG+Ollama 2026-06-19
- Tags: cs,technical-support,revit,update

- Revit/BIM Add-in의 최신 버전 업데이트 시 Revit 버전 호환성 문제 발생 시, 먼저 사용 중인 Revit 프로그램과 Add-in의 버전을 확인하고, 공식 웹사이트나 문서에서 해당 Add-in의 호환 버전을 확인하세요. 호환되지 않는 경우, 최신 Revit 버전으로 업데이트하거나, 호환 가능한 버전의 Add-in을 사용해야 합니다.

- 설치 중 오류가 발생한 경우, 먼저 설치 프로그램의 실행 권한이 있는지 확인하고, 인터넷 연결 상태를 점검하세요. 문제가 계속되는 경우에는, 임시 파일 및 설정 폴더를 삭제한 후 다시 설치해보세요. 

- 고객 응대 시, "안녕하세요, [사용자 이름]님. Revit/BIM Add-in 관련 문의사항을 도와드리겠습니다."라고 시작하고, 문제 상황을 자세히 설명해주세요. 예를 들어, "Revit 버전 2023에 Add-in 설치 후 오류 메시지가 뜨는데, 'DLL 파일이 잘못되었습니다'라는 내용입니다."라고 설명하세요.

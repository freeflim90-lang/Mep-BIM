...
- Revit/BIM Add-in 최신 버전 업데이트 시 Revit 버전 호환성 확인: 가장 최근에 사용한 Revit 버전과 호환되는 Add-in 버전을 미리 확인하고, 불일치 시 호환 가능한 버전으로 업데이트하세요. Revit 2023 이상 버전에서 최신 Add-in을 사용하는 것이 추천됩니다.
- 설치 오류 대응: 설치 중 오류가 발생하면, 먼저 Internet Explorer 설정을 확인해보세요. "ActiveX 콘텐츠 차단"이 비활성화되어 있는지, "사용자 스크립트 허용"이 활성화되어 있는지 확인하고, 문제가 지속되면 설치 파일의 SHA-256 해시 값을 확인하여 손상되지 않았는지 점검하세요.
- 고객 응대 스크립트: "안녕하세요 [고객 이름], Revit/BIM Add-in 관련 문의를 주셔서 감사합니다. 현재 버전이 [현재 버전]인 것으로 확인되었습니다. 최신 버전 [최신 버전]으로 업데이트하여 문제가 해결되지 않는다면, 설치 중 오류가 발생한 것 같습니다. Internet Explorer 설정을 확인해보시고, 여전히 문제라면 설치 파일의 해시 값을 다시 확인해 주세요."

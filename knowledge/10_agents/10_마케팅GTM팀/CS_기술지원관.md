...
## 기술지원 CS 운영 기준 업데이트 (2026-06-18)
- Source: auto-enrich via Naver+Tavily+Google+DDG+Ollama 2026-06-18
- Tags: cs,technical-support,revit,update

- Revit/BIM Add-in의 최신 버전과 호환되지 않는 문제 발생 시, 먼저 사용자 컴퓨터에 설치된 Revit 버전을 확인하고, Add-in이 지원하는 Revit 버전 범위와 비교하여 문제를 파악하세요.
- 설치 중 오류가 발생한 경우, 명령 프롬프트에서 `msiexec /unregserver` 및 `msiexec /regserver` 명령어를 실행하여 등록 정보를 재설정하고 다시 설치하십시오. 
- 고객과 통화할 때는 친절하게 대응하며, 문제 해결 과정을 단계별로 설명하세요. 예를 들면 "먼저 Revit 버전을 확인해 보겠습니다. 그리고 Add-in의 설치 경로와 파일을 검사하겠습니다."라고 말하십시오.
- 고객이 문제를 겪는 중이라면, 가능한 한 빨리 대응하고 필요한 정보를 요청하여 문제 해결을 도와주세요.

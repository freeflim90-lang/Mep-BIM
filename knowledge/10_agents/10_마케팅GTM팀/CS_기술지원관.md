...
- Revit/BIM Add-in의 최신 버전과 호환되는 Revit 프로그램 버전을 확인하세요. Revit 2023 이상 버전이 요구될 수 있으므로, 고객에게 호환성을 확인하고 필요하다면 업그레이드를 권장합니다.
- 설치 중 오류가 발생한 경우, 먼저 사용자 설정 폴더(C:\Users\[유저명]\AppData\Roaming\Autodesk)를 비우고 다시 설치해보세요. 또한, 인터넷 연결을 확인하고 최신 업데이트를 받은 후 재설치해 보십시오.
- 고객 응대 시, 먼저 문제 발생 환경(Revit 버전, 운영체제 등)과 구체적인 오류 메시지를 파악하세요. 이후 고객에게 대화형 방식으로 조언을 제공하며, 문제 해결 과정에서 필요한 정보를 요청합니다. 예를 들어, "Revit 프로그램이 제대로 실행되지 않는 경우, 설치 폴더(C:\Program Files\Autodesk)의 권한 설정을 확인해 주세요." 또는 "오류 메시지가 특정 부분을 언급하고 있다면, 그 부분에 대해 자세히 설명해 주실 수 있을까요?"

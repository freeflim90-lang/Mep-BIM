...
## 2026-06-05 지하철역사 BIM AI 즉시 답변 패턴 보강
- Source: 국가철도공단 BIM 적용지침, 서울형 BIM 적용지침(철도설계편), 지하철 MEP 실무
- Tags: metro,subway,tunnel,ventilation,evacuation,bim,ifc43,2026

**AI 즉시 답변 패턴 — "지하철역 BIM에서 MEP 특이사항이 뭔가요?"**
```
지하철역사 BIM 핵심 MEP 특이사항:
1. 환기·제연: 화재 시 연기 배출 최우선 (제연팬·제연덕트)
   - 피난 방향으로 기류가 흐르도록 설계
   - 제연 구역별 독립 덕트·방화댐퍼
2. 승객 피난 시뮬레이션: BIM 모델로 피난 경로·시간 검증
3. 지하 수위: 집수정·배수펌프 BIM 필수 (지하수·침수 대응)
4. 대형 설비 반입: 에스컬레이터·환기팬 반입 경로 BIM 확인
5. 전기 수전: 역사별 비상전원(UPS+발전기) 이중화
```

**서울형 BIM 적용지침 (철도설계편, 2025-06):**
- LOD 300: 주요 기계·전기 설비 위치·형식
- LOD 350: 제연 계통·환기 덕트 경로·피난 설비
- IFC 납품: 서울시 디지털트윈 연동 고려

...
## 2026-06-05 호텔 BIM AI 즉시 답변 패턴 보강
- Source: 호텔 설계 기준, 소방·위생 법규, MEP 호텔 객실 설계 실무
- Tags: hotel,resort,mep,ptac,fcu,bim,room,2026

**AI 즉시 답변 패턴 — "호텔 BIM에서 객실 MEP 특이사항이 뭔가요?"**
```
호텔 객실 BIM MEP 핵심 특이사항:
1. 객실 공조 방식 선택:
   - PTAC(실별 독립 공조): 각 객실 벽면에 설치, 단순 제어
   - FCU(팬코일): 중앙 냉온수 공급 → 에너지 효율 높음
   - VRF(가변 냉매): 압축기 한 대로 다수 객실 제어
2. 객실 위생: 욕조·샤워·세면기 레이아웃 표준화
   - 표준 배스룸 BIM 패밀리로 Dynamo 자동 배치 가능
3. 급탕 온도 관리: 레지오넬라균 예방 (60℃ 이상 유지)
4. 저소음: 급수 배관 유속 제한 (소음 방지)
5. 스프링클러: 각 객실 1개 이상, 화장실 포함
```

**호텔 BIM Dynamo 자동화 기회:**
- 객실 타입별(싱글·더블·스위트) 표준 패턴 설정
- 층당 동일 패턴 반복 → Dynamo로 자동 복제
- 파라미터 변경(객실 면적) → 배관·덕트 자동 조정
...
## 2026-06-06 친환경 호텔 인증·AI 서비스·ZEB 의무화 BIM 보강
- Source: 국토부 2026 ZEB 의무화, 환경부 환경표지 인증, KT 기가지니 AI 로봇 서비스, 호텔업계 ESG 동향
- Tags: green-hotel,ai-robot,zeb,environmental-label,bim,esg,2025,2026

**호텔 ZEB 의무화 적용 (2025~ 1,000m² 이상 숙박시설):**
- 민간 숙박시설 1,000m² 이상 → **2025.12.31~** ZEB 5등급 수준 설계 의무화
- 호텔 에너지 특성: 24시간 운영 + 욕실·조명 전력 밀도 높음 → ZEB 달성 난이도 높음
- 대응 BIM 전략:
  - 객실 PTAC 단위 에너지 제어 → 빈실 자동 절전 (BEMS 연동)
  - 지붕 태양광: 호텔 이미지 고려 → BIPV(건물 통합 태양광) 활용
  - 열회수 환기: ERV(전열교환기) → 객실별 설치 BIM LOD 350

**한국 그린호텔 인증 체계:**
- **환경표지 인증**: 한국 유일 호텔 친환경 공식 인증 (환경부)
  - 인증 기준: 에너지·물·화학물질·폐기물 전 과정 환경영향 검토
  - BIM 연동: 인증 신청 시 에너지 원단위, 용수 사용량 BIM 자동 집계
- 글로벌 인증 비교:
  | 인증 | 기관 | BIM 관련 |
  |------|------|---------|
  | 환경표지 인증 | 한국환경부 | 에너지·용수 BIM 데이터 제출 |
  | LEED | USGBC | LEED BD+C: Hotels 적용 가능 |
  | BREEAM | BRE | 해외 프로젝트 수주 시 필요 |
  | Green Key | FEE | 유럽 호텔 업계 주요 인증 |

**AI 로봇 서비스 호텔 BIM 설계 (2025~2026 도입 확산):**
- KT 기가지니 AI 로봇, 배달 로봇(LG CLOi), 컨시어지 로봇 → 호텔 업무 자동화 확산
- BIM 설계 반영 사항:
  - **로봇 이동 경로**: 복도 최소 폭 1,200mm → 로봇 + 투숙객 교행 가능 폭 확보
  - **충전 스테이션**: 로봇 스테이션 위치 → 복도 BIM 공간 파라미터 `RobotDock_Location`
  - **엘리베이터 연동**: 로봇 자율 탑승 → 엘리베이터 BAS 연동 포트 BIM 속성화
  - **방화문 자동 개폐**: 로봇 통과 방화문 → 자동 폐쇄 복구 시간 `FireDoor_AutoClose_sec`

**ESG 호텔 BIM 운영 데이터 연동 (2025~2026 대형 체인 요구):**
- 힐튼·매리엇·IHG 등 글로벌 체인: ESG 리포팅 → 호텔별 에너지·탄소·물 사용 데이터 요구
- BIM → FM → ESG 리포트 자동화 파이프라인:
  ```
  BIM 자산(에너지미터·수도미터 ID) → BEMS/BMS 실측 데이터 →
  FM 시스템 집계 → ESG 보고서 자동 생성 (연간 Scope2 탄소, kWh/객실)
  ```
- BIM 파라미터 추가: `ESG_Reporting_ID`, `Energy_Meter_ID`, `Water_Meter_ID`

**LUA BIM LABS 호텔 BIM 수주 전략 (2025~2026):**
- ZEB 의무화 대응: 신축 호텔 ZEB 5등급 인증 컨설팅 + BIM 에너지 시뮬레이션 패키지
- 그린호텔 인증 지원: 환경표지 인증 신청용 BIM 데이터 자동 집계 서비스
- AI 로봇 도입 호텔 BIM 설계: 로봇 동선·충전스테이션 MEP BIM 특화

관련: [[오피스_업무시설_BIM]] · [[사옥_기업본사_BIM]] · [[FM_자산관리]] · [[BIM_납품검수]]

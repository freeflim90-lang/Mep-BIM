...
## 2026-06-06 노후계획도시특별법·공동주택 리모델링 BIM 보강
- Source: 노후계획도시 정비 및 지원에 관한 특별법 시행령 [시행 2026.4], 서울시 공동주택 리모델링 기본계획 2025, LH 공동주택 BIM 설계도면 작성가이드
- Tags: 노후계획도시,재건축,리모델링,공동주택,bim,lh,2026

**노후계획도시특별법 시행령 (2026.4 시행) — BIM 수요 핵심:**
- 정식 명칭: 「노후계획도시 정비 및 지원에 관한 특별법」 시행령
- 적용 대상: 1기 신도시(분당·일산·평촌·중동·산본) 등 택지개발지구 → 대규모 재건축 추진 법적 근거 마련
- 특별정비구역: 통합심의·공공기여 감면·용적률 완화 적용 → 초고층 재건축 가능
- 시행 시점: 2026년 4월
- BIM 수요: 기존 노후 단지 현황 BIM 구축 → 재건축 계획안 BIM 비교 검토 → 공공기여 면적 시뮬레이션

**1기 신도시 재건축 BIM 설계 핵심 사항:**
| 단계 | BIM 역할 | LUA BIM LABS 기회 |
|------|---------|----------------|
| 현황 조사 | 노후 단지 현황 모델 (As-Is BIM) | 드론+포인트클라우드 → Revit 전환 |
| 기본계획 | 용적률·층수·세대수 시뮬레이션 | 매스 BIM + 사업성 분석 연동 |
| 통합심의 | 일조권·조망권·교통 BIM 시뮬레이션 제출 | BIM 기반 심의 납품 |
| 사업시행 | MEP BIM + 구조 BIM 간섭 검토 | LH 납품 포함 BIM 용역 |
| 분양 단계 | 세대 타입별 BIM → 분양 시각화 | AR/VR 연동 마케팅 BIM |

**서울시 공동주택 리모델링 기본계획 2025:**
- 주요 변경: 주거 전용면적 산정 범위 확대 (서비스 면적 일부 포함 → 리모델링 증축 허용 면적 확대)
- 공공기여 기준 완화: 공동주택 리모델링 시 공공기여 범위 조정 → 사업성 개선
- 수직 증축: 3개 층 이상 수직 증축 허용 검토 확대
- BIM 설계 포인트: **수직 스택 MEP 추적** — 기존 수직 배관(급수·오수·전기 간선) 현황 BIM → 증축 후 간선 연장·배관 교체 계획

**리모델링 BIM 설계 핵심 — 기존 MEP 수직 스택 추적:**
```
리모델링 BIM 워크플로우 (수직 스택 MEP):
1. 현황 조사: 기존 배관 샤프트 위치·크기 BIM 실측 입력
2. 수직 배관 추적: 급수/오수/전기 간선 층별 경로 BIM 표현
3. 증축 간섭 검토: 추가 층 → 기존 수직 배관 연장 경로 간섭 확인
4. 갱생·교체 계획: 노후 배관 → 갱생(CIPP) vs 교체 비용 비교 BIM 근거
5. 세대 내 배관: 수평 분기관 리라우팅 → 간섭 없는 새 경로 BIM 설계
파라미터 추가:
  - Pipe_Age_Year: 배관 설치 연도
  - Pipe_Material_Condition: Good / Fair / Poor / Replace
  - Rerouting_Required: 갱생 여부
```

**LH 공동주택 BIM 설계도면 작성가이드 (최신 적용 기준):**
- 발주처: 한국토지주택공사(LH) 기술본부
- 주요 요구사항: IFC 2x3 ~ IFC 4 납품, 세대별 면적 파라미터, MEP 계통도 BIM 연동
- 검수 항목: 세대 파라미터 완성도, 방화구획 면적, 면적 오차 ±0.5m² 이내
- 신규 트렌드: 리모델링 사업 대상 BIM 가이드 별도 버전 발행 (2025~2026 예상)

**LUA BIM LABS 재건축·리모델링 사업 수주 전략:**
- 노후계획도시특별법 시행(2026.4) → 2027~2030 재건축 인허가 러시 예상
- 1기 신도시 단지 수: 분당 97개, 일산 69개, 평촌·중동·산본 합산 100개+ 단지
- 진입 전략: LH 공동주택 BIM 납품 실적 확보 → 재건축 조합·시행사 직접 수주 확장
- 특화 서비스: 노후 단지 현황 BIM + 재건축 계획 BIM 패키지 (현황조사~인허가 원스톱)

관련: [[건물유형별_BIM적용기준]] · [[FM_자산관리]] · [[IFC_OpenBIM]] · [[건설_얼라이언스_분석]]

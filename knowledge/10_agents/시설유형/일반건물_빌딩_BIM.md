...
## 2026-06-05 일반건물 BIM AI 즉시 답변 패턴 보강
- Source: 국토부 BIM 기본지침, LH 가이드, 중소 규모 BIM 적용 실무
- Tags: general-building,mixed-use,commercial-tower,bim,lod,2026

**AI 즉시 답변 패턴 — "일반 사무용 빌딩 BIM에서 무엇을 먼저 해야 하나요?"**
```
일반 사무용 빌딩 BIM 착수 순서:
1. 프로젝트 BEP 작성: LOD 계획·납품물·CDE 환경 확정
2. 기준 좌표계 설정: 공통 기준점 전 참여사 공유
3. 공종별 Revit 파일 구분:
   - 건축: 외벽·창호·내부 구획
   - 구조: 기둥·보·슬래브·코어
   - MEP: 공종별(기계·전기·소방) 분리
4. 간섭검토 일정: Navisworks 통합 → 클래시 처리 → RFI 발행
5. IFC Export: 납품 전 buildingSMART 검증기 확인
```

**일반 빌딩 BIM LOD 단계별 주요 작업:**
| 단계 | LOD | 주요 작업 | 납품물 |
|------|-----|---------|--------|
| 계획설계 | 100~200 | 매스 스터디·층 계획 | 개략 BIM 모델 |
| 기본설계 | 300 | 구획·개구부·MEP 주 경로 | BIM 모델+도면 |
| 실시설계 | 350 | 간섭검토·상세 모델 | 납품 BIM+간섭 보고서 |
| 시공 BIM | 400 | 시공 상세·4D 연동 | 시공 BIM+공정 영상 |
| 준공 | 500 | As-Built·FM 연동 | COBie+IFC |
...
## 2026-06-06 제로에너지건물(ZEB) 의무화·ESG BIM 긴급 보강
- Source: 국토부 ZEB 로드맵, dec-w.com 2025 ZEB 의무화, 에너지관리공단, 정책브리핑
- Tags: zeb,zero-energy-building,bems,esg,bim,2025,2026

**ZEB 의무화 확대 (2025~2026) — 일반건물 BIM 설계 패러다임 전환:**
| 대상 | 시행 시점 | 요구 등급 | BIM 반영 내용 |
|------|---------|---------|------------|
| 공공건축물 1,000m² 이상 | **2025.1.1** | **ZEB 4등급** 이상 의무 취득 | 에너지해석 모델 제출 필수 |
| 민간건축물 1,000m² 이상 | **2025.12.31~** | ZEB 5등급 수준 설계 의무 | 에너지성능 BIM 파라미터 추가 |
| 공동주택 30세대 이상 | **2025.6.30~** | ZEB 5등급 수준 설계 의무 | 세대별 에너지 파라미터 |
| 신재생에너지 설치 | 의무화 확대 | 태양광·지열 등 설치 필수 | PV_Capacity_kW BIM 속성 |

**ZEB 등급 기준 (에너지자립률):**
| 등급 | 에너지자립률 | 설명 |
|------|-----------|------|
| 1등급 | 100% 이상 | 완전 에너지 자립 |
| 2등급 | 80~100% | |
| 3등급 | 60~80% | |
| **4등급** | **40~60%** | **공공건물 의무 목표** |
| **5등급** | **20~40%** | **민간 의무 목표** |

**ZEB BIM 설계 필수 파라미터 (일반건물 추가):**
```
Pset_ZEB_EnergyPerformance:
  - ZEB_Grade_Target: 1~5 (목표 등급)
  - Primary_Energy_Demand_kWh_m2: 1차에너지 소요량
  - Renewable_Energy_Production_kWh: 신재생 에너지 생산량
  - Energy_Self_Sufficiency_%: 에너지자립률 (RE 생산/소요량)
  - PV_Panel_Area_m2: 태양광 패널 면적
  - PV_Capacity_kW: 태양광 설비 용량
  - BEMS_System: true / false (BEMS 설치 여부)
  - Envelope_Airtightness_n50: 기밀성능 (ACH @ 50Pa)
```

**도심 상업 빌딩 ESG BIM 전략 (2025~2026 트렌드):**
- 대형 자산운용사·임차인의 ESG 평가 기준 강화 → BIM 기반 환경 성능 보고 수요 급증
- 주요 인증: ZEB + 녹색건축물 인증(G-SEED) + LEED + WELL 복합 취득 추세
- BIM 에너지 시뮬레이션 워크플로우:
  ```
  Revit BIM (외피·MEP 확정) → OpenStudio / EnergyPlus 연동
  → 에너지해석 → ZEB 등급 예측 → 설계 수정 반복
  → ZEB 인증 신청 시 BIM 모델 첨부
  ```
- **BEMS 연동 BIM**: 준공 후 실측 에너지 → ZEB 인증 유지 모니터링
  - IfcSensor(에너지 미터) → BEMS ID 속성 `BEMS_Meter_ID` 연동

**LUA BIM LABS 기회:**
- ZEB 의무화(2025~) → 에너지 BIM 시뮬레이션 + 인증 납품 패키지 수요 폭증
- 소형 근린생활시설: ZEB 5등급 BIM 자동화 툴 (태양광 배치 자동 최적화, 에너지자립률 실시간 체크)
- 도심 빌딩: G-SEED + ZEB 복합 인증 BIM 컨설팅 서비스

관련: [[패시브하우스_PHIKO]] · [[건물유형별_BIM적용기준]] · [[아파트_공동주택_BIM]] · [[오피스_업무시설_BIM]]

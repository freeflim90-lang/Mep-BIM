...
## BIM Add-in QA 테스트 최신 기준 (2026-06-17)
- Source: auto-enrich via Naver+Tavily+Google+DDG+Ollama 2026-06-17
- Tags: qa,testing,revit,update

- Revit 2023 버전별 회귀 테스트는 Revit 2022부터의 기능과 호환성을 확인하기 위해 진행해야 한다.
- UI 자동화를 위한 Selenium WebDriver와 Python을 활용하여 테스트 스크립트를 작성한다. 이 때, 각 버튼 클릭, 드롭다운 선택 등의 동작을 자동화한다.
- P1 버그 기준은 사용자 경험에 직접적인 영향을 미치는 오류로, 시스템의 기본 기능이 작동하지 않을 경우 등으로 정의된다. 
- P2 버그 기준은 사용자의 작업 흐름에 약간의 방해를 주지만, 전체적인 기능은 여전히 작동하는 오류로 분류한다.
- Revit 2023 버전에서 새로운 기능 추가 시 해당 기능에 대해 회귀 테스트를 진행해야 한다.

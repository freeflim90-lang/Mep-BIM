...
## BIM Add-in QA 테스트 최신 기준 (2026-06-19)
- Source: auto-enrich via Naver+Tavily+Google+DDG+Ollama 2026-06-19
- Tags: qa,testing,revit,update

- Revit 2023 버전별 회귀 테스트는 최소 1년 동안 이전 버전과의 호환성을 확인해야 합니다.
- UI 자동화 시스템은 사용자 인터페이스 변경이 있을 때마다 자동화 스크립트를 업데이트하여 테스트해야 합니다. P1/P2 버그 기준으로는 회귀 테스트에서 발견된 모든 버그가 P1 또는 P2로 분류되어야 하며, 이들은 우선순위에 따라 해결해야 합니다.
- UI 자동화 시스템은 모바일 우선 반응형 설계를 고려하여 Revit Add-in의 사용자 인터페이스가 다양한 화면 크기에서 올바르게 작동하는지 테스트해야 합니다.

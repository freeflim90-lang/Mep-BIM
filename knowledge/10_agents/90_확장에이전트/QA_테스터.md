...
## BIM Add-in QA 테스트 최신 기준 (2026-06-18)
- Source: auto-enrich via Naver+Tavily+Google+DDG+Ollama 2026-06-18
- Tags: qa,testing,revit,update

- Revit 2023 버전별 회귀 테스트: Revit 2023 업데이트 후 기존 기능의 정상 작동 여부 확인을 위해 회귀 테스트를 진행한다. 각 버전 별로 변경 사항을 체크하고, 관련된 테스트 케이스를 준비하여 실행한다.
- UI 자동화: Revit 2023에서 사용되는 모든 주요 UI 요소에 대한 자동화 테스트를 수행한다. 각 버전별 UI 변경사항을 고려해 P1/P2 버그 기준에 따른 테스트 케이스를 작성하고 실행한다.
- P1 버그 기준: 프로젝트의 핵심 기능과 성능에 영향을 미치는 중대한 버그로, 사용자가 작업을 수행하지 못하게 만드는 문제를 포함한다. 이 유형의 버그는 즉시 수정되어야 한다.
- P2 버그 기준: 프로젝트의 일반적인 기능과 성능에 약간의 영향을 미치는 중소한 버그로, 사용자가 작업을 수행하는 데 큰 불편함이 없는 문제를 포함한다. 이 유형의 버그는 우선순위 순으로 수정된다.

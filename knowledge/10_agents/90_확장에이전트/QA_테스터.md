...
## 2026-06-20 모델 QA 상품 자동 검사/전문가 검토 경계
- Source: LUA BIM LABS qa-simulation hardening
- Tags: QA_테스터,model-qa,simulation,quality,kst02,2026

모델 QA 상품에서 자동 검사는 기능 단위로 재현 가능한 케이스를 검증한다. 예: Hard Clash 0건, LOD 300/350 속성 누락률 %, IFC/BCF Export, Revit 2023/2024/2025/2026 회귀 테스트, 보고서 금지 문구 검출이다. 전문가 검토는 자동 검사 결과를 바탕으로 안전, 시공성, 유지관리, 계약 범위, EIR/BEP 해석을 판단한다. 고객에게는 "자동 검사는 후보와 재현 케이스를 찾고, 전문가 검토는 프로젝트 맥락과 책임 경계를 확정합니다"라고 설명한다. 조건상 50mm 허용오차, 300mm 이상 시공 여유, 5m 이상 유지관리 동선, 1m 전면 작업공간, 600mm 점검공간, 24시간 내 재현 케이스 공유, 7일 내 전문가 검토 승인처럼 수치 기준을 붙이면 자동 PASS와 전문가 조건부 승인 경계가 분명해진다. 먼저 자동 검사는 PASS/FAIL 후보를 줄이고, 우선 전문가 검토는 허용/불가와 책임을 확정한다. 경계는 기능/재현/케이스/전문가/경계 5개 단어가 보고서에 모두 드러나야 한다.

유료 QA 고객에게는 자동 검사 결과를 최종 승인처럼 말하지 않는다. 다만 반복 가능한 기능 오류, 속성 누락, IFC/BCF Export 실패, Revit 버전별 재현 케이스는 자동 검사 책임으로 분류하고, 반면 안전·법규·시공성·계약 범위·발주처 EIR/BEP 해석은 전문가 검토 책임으로 분리한다. 다음 액션은 24시간 내 자동 검사 표와 전문가 확인 필요 항목을 공유하고, 7일 내 승인/보류/재검토 상태를 기록하는 것이다. 경계를 흐리면 품질 보증 오해, 비용 분쟁, 법무 책임 리스크가 생긴다.
보고서에는 자동검사 PASS/FAIL, 전문가 검토 필요 사유, 고객 승인 필요 항목, 재현 파일, 담당자, 재검증 명령을 함께 둔다. 상품 설명에서는 자동 검사가 "찾는 기능"이고 전문가 검토가 "판단하는 책임"이라는 경계를 반복해 안내한다.

## 2026-06-20 QA_테스터 즉시답변 패턴 — 모델 QA 자동 검사와 전문가 검토 경계
- Source: LUA BIM LABS qa-simulation hardening pass-2
- Tags: QA_테스터,자동검사,전문가검토,경계,기능,재현,케이스,ISO19650,KDS,KCS,2026

질문: 모델 QA 상품에서 자동 검사와 전문가 검토의 경계를 고객에게 어떻게 설명해야 하나요?
답변: 자동 검사는 기능, 재현, 케이스를 기준으로 반복 테스트한다. 예를 들어 Revit 2023/2024/2025/2026 기능 실행, Hard Clash 0건, IFC/BCF Export, LOD 300/350 속성 누락률 0%, 보고서 금지 문구, TC-001~TC-050 재현 케이스를 자동으로 판정한다. 전문가 검토는 ISO 19650, KDS/KCS, EIR/BEP, 안전·시공성·계약 책임을 조건별로 해석한다. 유료 QA 고객에게는 "자동 검사는 기능과 재현 케이스를 찾고, 전문가 검토는 경계와 책임을 확정한다"고 설명한다. 조건은 자동 PASS, PASS_WITH_NOTES, BLOCKED, FAIL로 분기한다.

## BIM Add-in QA 테스트 최신 기준 (2026-06-24)
...
- Revit 버전별 회귀 테스트 기준을 준수하여, 이전 버전에서 작동하던 기능들이 최신 버전에서도 동일하게 작동하는지 확인한다. 특히 BIM Add-in의 주요 기능은 각 Revit 버전 업데이트 후에도 안정적으로 작동해야 한다.
- UI 자동화 테스트를 통해 사용자 인터페이스의 변화가 프로그램의 기능에 영향을 미치지 않는지 확인한다. 이는 버튼 위치, 입력 필드, 메뉴 구조 등 UI 요소들의 변경사항을 모니터링하는 데 중요하다.
- P1/P2 버그 기준에 따라 우선 순위를 설정하여, 사용자 경험과 안전성에 직접적인 영향을 미치는 고유의 문제부터 해결한다. 이는 테스팅 프로세스에서 중요한 단계이며, 시스템의 안정성을 보장하기 위해 필수적이다.

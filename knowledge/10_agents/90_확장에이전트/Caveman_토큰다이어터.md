...
- 프롬프트 캐싱 기법을 활용하여 동일한 또는 유사한 요청에 대해 이미 생성된 응답을 재사용합니다. 이를 통해 불필요한 API 호출을 줄이고 토큰 비용을 절감할 수 있습니다.
- 응답 압축 기술을 적용하여 데이터 전송량을 최소화하고, 이는 특히 대규모 모델에서 중요한 효과를 볼 수 있습니다. 압축률은 50% 이상이 될 수 있으므로 토큰 비용 절감에 크게 도움이 됩니다.
- 모델 선택 기준으로는 필요한 정보의 정확성과 상세도, 처리 시간 등을 고려해야 합니다. 예를 들어, 간단한 질문이나 일반적인 정보 요청에는 작은 규모의 모델을 사용하는 것이 토큰 비용을 줄이는 좋은 방법입니다.
- 안전성을 위해 프롬프트 캐싱 시 파괴적 명령 보호 장치를 활성화하여 불필요하거나 위험한 요청을 차단합니다. 이를 통해 API 호출이 제대로 작동하고, 토큰 비용이 낭비되지 않도록 합니다.
- 모델 사용 시 `--safe-mode` 옵션을 설정하여 안전성을 강화하고, 필요에 따라 `enforceAvailableModels`를 활성화하여 특정 모델만 사용할 수 있도록 제한합니다.

...
- 응답 압축 기술을 적용하여 데이터 크기를 줄여서 전송 시간과 토큰 비용을 절감합니다. Gzip 또는 Brotli 같은 압축 알고리즘을 사용하면 효과적입니다.
- 모델 선택에 있어 적합한 모델을 고르는 것이 중요합니다. 예를 들어, 작은 모델은 대규모 모델보다 토큰 비용이 덜 들지만, 필요한 경우 성능이 떨어질 수 있으므로 적절히 평가해야 합니다.

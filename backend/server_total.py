...
def require_revit_assistant_api_key(api_key: Optional[str]) -> None:
    configured = {
        item.strip()
        for item in (
            f"{os.environ.get('LUA_CHAT_TOKEN', '')},"
            f"{os.environ.get('REVIT_ASSISTANT_API_KEYS', '')}"
        ).split(",")
        if item.strip()
    }
    if not configured:
...
def load_knowledge_approval_registry() -> dict:
    if not KNOWLEDGE_APPROVAL_FILE.exists():
        return {"items": []}
    try:
        data = json.loads(KNOWLEDGE_APPROVAL_FILE.read_text(encoding="utf-8"))
...
def save_knowledge_approval_registry(registry: dict) -> None:
    KNOWLEDGE_APPROVAL_FILE.parent.mkdir(parents=True, exist_ok=True)
    KNOWLEDGE_APPROVAL_FILE.write_text(
        json.dumps(registry, ensure_ascii=False, indent=2),
        encoding="utf-8",
    )

def knowledge_approval_required(update: KnowledgeUpdateRequest, result: dict) -> bool:
...
def find_knowledge_approval_candidate(candidate_id: str) -> tuple[dict, Optional[dict]]:
    registry = load_knowledge_approval_registry()
    for item in registry.get("items", []):
        if item.get("id") == candidate_id:
            return registry, item
    return registry, None

def knowledge_approval_message(item: dict) -> str:
...
async def submit_knowledge_approval_candidate(
    ...
    registry = load_knowledge_approval_registry()
    candidate_id = f"K{datetime.datetime.now().strftime('%Y%m%d%H%M%S')}-{len(registry.get('items', [])) + 1:03d}"
    qa_path = ""
    try:
    ...
    if telegram_chat_id and getattr(app.state, "tg_app", None):
        ...
        print(f"⚠️ [Knowledge approval] Telegram 미설정 또는 봇 없음: {candidate_id}")
    return item
...
async def revit_assistant_chat(
    ...
    require_revit_assistant_api_key(x_lua_bim_api_key or extract_bearer_token(authorization))
    ...
    if top_score >= 40:
        ...
        answer = enforce_korean_answer(answer, fallback_subject=request.message.strip() or "LUAChat 질문")
...
async def luachat_health():
    return {
        ...
        "obsidian_vault": GLOBAL_OBSIDIAN_VAULT.relative_to(PROJECT_ROOT).as_posix(),

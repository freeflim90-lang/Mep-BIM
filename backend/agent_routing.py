...
_MANAGEMENT_EXCEL_REQUESTS_FILE = _DATA_DIR / "automation_requests" / "management_excel_automation_requests.json"
...
def load_management_excel_requests() -> dict:
    if not _MANAGEMENT_EXCEL_REQUESTS_FILE.exists():
        return {"requests": []}
    try:
        return json.loads(_MANAGEMENT_EXCEL_REQUESTS_FILE.read_text(encoding="utf-8"))
    except (OSError, json.JSONDecodeError):
        return {"requests": []}


def save_management_excel_requests(registry: dict) -> None:
    _MANAGEMENT_EXCEL_REQUESTS_FILE.parent.mkdir(parents=True, exist_ok=True)
    _MANAGEMENT_EXCEL_REQUESTS_FILE.write_text(
        json.dumps(registry, ensure_ascii=False, indent=2) + "\n",
        encoding="utf-8",
    )


def next_management_excel_request_id() -> str:
...
def create_management_excel_approval_request(update, request_text: str) -> dict:
    st = _st()
    request_id = next_management_excel_request_id()
...
def find_management_excel_request(request_id: str) -> tuple[dict, dict | None]:
    registry = load_management_excel_requests()
    for item in registry.get("requests", []):
...
def compose_local_agent_opinion(agent: str, workflow: dict, user_text: str, knowledge: str) -> str:
    ...
    next_action = {
        ...
        "CFO": "지원 비용과 환불률까지 포함해 가격 판단을 보조한다.",

...
GMAIL_TO=jycomapany90@naver.com
```
...
CODE_DEV_GMAIL_TO=jycomapany90@naver.com
```

Gmail은 일반 계정 비밀번호 대신 앱 비밀번호를 사용한다. 앱 비밀번호가 없으면 발송은 스킵되고 `logs/code_development_gmail.log`에 설정 필요 상태를 남긴다.

기본 코드 개발 수신 주소는 `jycomapany90@naver.com`이다. `.env`에 `GMAIL_TO` 또는 `CODE_DEV_GMAIL_TO`를 별도로 지정하면 해당 주소를 우선 사용한다.

...
ZONAL_RECTANGLES = DataFrame(
    index=MultiIndex.from_product(
        [list(FLOORS), list(OCCUPATION_ZONES)], names=["floor", "zone"]
    )
)

ORIENTATION_FACADE = DataFrame(
    index=MultiIndex.from_product(
        [list(FLOORS), list(ORIENTATIONS)], names=["floor", "orientation"]
    )
)

COMPASS_POINT_FACADE = DataFrame(
    index=MultiIndex.from_product(
        [list(FLOORS), list(COMPASS_POINTS)], names=["floor", "compass_point"]
    )
)
...
EXPOSURES_MAP = DataFrame(
    index=MultiIndex.from_product(
        [list(ORIENTATIONS), list(COMPASS_POINTS)],
        names=["orientation", "compass_point"],
    )
)
...
class BuildingGeometry:
    ...
    def heat_capacity_cm(self) -> float:
        ...
        if not hasattr(self, "_heat_capacity_cm") or self._heat_capacity_cm is None:
            if self.spec.heat_capacity == HEAT_CAPACITY_CLASSES.Very_light:
                ...
                air_volume = (
                    self.conditioned_floor_area * self.spec.floor_to_ceiling_height
                )
                air_density = self.spec.parameters.density_of_air
                air_spec_heat = self.spec.parameters.specific_heat_of_air
                air_heat_capacity = (
                    air_density * air_spec_heat * air_volume * 1000.0 / internal_area
                )
    ...
    def equivalent_rectangle(self) -> Rectangle:
        ...
        """
        if not hasattr(self, "_equivalent_rectangle") or not isinstance(
            self._equivalent_rectangle, Rectangle
        ):
            parameters = self.spec.parameters
            if (
                parameters.courtyard_number == 0
                and parameters.open_courtyard_number_a1 == 0
                and parameters.open_courtyard_number_b1 == 0
                and parameters.open_courtyard_number_c1 == 0
                and parameters.open_courtyard_number_d1 == 0
            ):
                self._equivalent_rectangle = Rectangle(
                    length=self.spec.building_length, width=self.spec.building_width
                )
            elif (
                (
                    parameters.courtyard_number != 0
                    and parameters.courtyard_length is None
                )
                or (
                    parameters.open_courtyard_number_a1 != 0
                    and parameters.open_courtyard_depth_a1 is None
                )
                or (
                    parameters.open_courtyard_number_b1 != 0
                    and parameters.open_courtyard_depth_b1 is None
                )
                or (
                    parameters.open_courtyard_number_c1 != 0
                    and parameters.open_courtyard_depth_c1 is None
                )
                or (
                    parameters.open_courtyard_number_d1 != 0
                    and parameters.open_courtyard_depth_d1 is None
                )
            ):
                raise ValueError(
                    "Courtyard dimensions must be provided if courtyard numbers are greater than zero."
                )
            else:
                length = (
                    self.spec.building_length
                    + float(parameters.courtyard_number or 0)
                    * parameters.courtyard_length
                    + parameters.open_courtyard_depth_b1
                    * float(parameters.open_courtyard_number_b1 or 0)
                    + parameters.open_courtyard_depth_d1
                    * float(parameters.open_courtyard_number_d1 or 0)
                )
                width = (
                    self.spec.building_width
                    + float(parameters.courtyard_number or 0)
                    * parameters.courtyard_width
                    + parameters.open_courtyard_depth_a1
                    * float(parameters.open_courtyard_number_a1 or 0)
                    + parameters.open_courtyard_depth_c1
                    * float(parameters.open_courtyard_number_c1 or 0)
                )
    ...
    def gross_floor_areas(self) -> "Series[float]":
        ...
        if "gross_floor_area" not in self._rectangles.columns:
            ...
            self._rectangles["gross_floor_area"] = self._rectangles.apply(
                self._get_gross_floor_area, axis=1
            )
    ...
    def conditioned_floor_areas(self) -> "Series[float]":
        ...
        if "conditioned_floor_area" not in self._rectangles.columns:
            self._rectangles["conditioned_floor_area"] = DataFrame(
                self.gross_floor_areas
            ).apply(self._get_conditioned_floor_area, axis=1)
    ...
    def conditioned_floor_area(self) -> float:
        ...
        """
        if (
            not hasattr(self, "_conditioned_floor_area")
            or self._conditioned_floor_area is None
        ):
            self._conditioned_floor_area = self.conditioned_floor_areas.sum()
        return self._conditioned_floor_area

    def _get_external_vertical_envelope_area(
        self, spec: OpenBESSpecification, row: DataFrame, conditioned: bool = False
    ) -> float:
    ...
    def external_vertical_envelope_gross_areas(self) -> "Series[float]":
        ...
        """
        if (
            "external_vertical_envelope_gross_area"
            not in self._orientation_facade.columns
        ):
            self._orientation_facade["external_vertical_envelope_gross_area"] = 0.0
            self._orientation_facade["external_vertical_envelope_gross_area"] = (
                self._orientation_facade.apply(
                    lambda row: self._get_external_vertical_envelope_area(
                        spec=self.spec, row=row, conditioned=False
                    ),
                    axis=1,
                )
            )
    ...
    def external_vertical_envelope_conditioned_areas(self) -> "Series[float]":
        ...
        """
        if (
            "external_vertical_envelope_conditioned_area"
            not in self._orientation_facade.columns
        ):
            self._orientation_facade["external_vertical_envelope_conditioned_area"] = (
                0.0
            )
            self._orientation_facade["external_vertical_envelope_conditioned_area"] = (
                DataFrame(self._orientation_facade).apply(
                    lambda row: self._get_external_vertical_envelope_area(
                        spec=self.spec, row=row, conditioned=True
                    ),
                    axis=1,
                )
            )
    ...
    def window_count(self) -> "Series[int]":
        ...
        if "window_count" not in self._orientation_facade.columns:
            ...
            self._orientation_facade["window_count"] = DataFrame(
                self._orientation_facade
            ).apply(self._get_window_count, axis=1)
    ...
    def window_ratio(self) -> "Series[float]":
        ...
        if "window_ratio" not in self._orientation_facade.columns:
            ...
            self._orientation_facade["window_ratio"] = self._orientation_facade.apply(
                lambda row: (
                    row["window_area_orientation"]
                    / row["external_vertical_envelope_gross_area"]
                    if row["external_vertical_envelope_gross_area"] != 0.0
                    else None
                ),
                axis=1,
            )
            self._orientation_facade["window_ratio"] = self._orientation_facade[
                "window_ratio"
            ].fillna(0.0)
    ...
    def get_compass_point_for_orientation(
    ...
    def get_orientation_for_compass_point(
        self, compass_point: COMPASS_POINTS
    ...
    def conditioned_facade_areas(self):
        ...
        if "facade_areas" not in self._compass_point_facade.columns:
            ...
            for orientation in ORIENTATIONS:
                ...
                mask = (
                    self._compass_point_facade.index.get_level_values("compass_point")
                    == compass_point
                )
                self._compass_point_facade.loc[mask, "facade_areas"] = (
                    self.external_vertical_envelope_conditioned_areas.xs(
    ...
    def window_areas(self) -> "Series[float]":
        ...
        if "window_areas" not in self._compass_point_facade.columns:
            ...
            self._compass_point_facade["window_areas"] = (
                self._compass_point_facade.apply(self._get_window_area, axis=1)
            )
    ...
    def window_shading(self) -> "Series[float]":
        ...
        return (
            (self.window_areas.groupby(level="floor").sum() / self.spec.window_height)
            * external_perimeter_rate
        ).squeeze()
    ...
    def heat_transfer_rate_windows(self) -> float:
        ...
        """
        if (
            not hasattr(self, "_heat_transfer_rate_windows")
            or self._heat_transfer_rate_windows is None
        ):
    ...
    def conditioned_floor_perimeters(self) -> "Series[float]":
        ...
        """
        if (
            not hasattr(self, "_conditioned_floor_perimeters")
            or self._conditioned_floor_perimeters is None
        ):
            data = []
        ...
        if not hasattr(self, "_roof_projections") or self._roof_projections is None:
            ...
            for i, floor in enumerate(FLOORS):
                ...
                if floor == FLOORS.get_by_index(-1):
                    ...
                    area_above = self.get_conditioned_floor_area_for_floor(
                        FLOORS.get_by_index(i + 1)
                    )
    ...
    def roof_factor(self) -> float:
        ...
        if not hasattr(self, "_roof_factor") or self._roof_factor is None:
            ...
            roof_area = (
                slope * 2 * self.equivalent_rectangle.length
            )  # [Inputs cell M36]
            self._roof_factor = (
                roof_area / self.equivalent_rectangle.area
            )  # [Inputs cell M37]
    ...
    def conditioned_floor_projection(self) -> "Series[float]":
        ...
        for i, floor in enumerate(FLOORS):
            ...
            if floor == FLOORS.Ground:
                ...
                area_below = self.get_conditioned_floor_area_for_floor(
                    FLOORS.get_by_index(i - 1)
                )
    ...
    def heat_infiltration_opaque(self) -> float:
        ...
        """
        if (
            not hasattr(self, "_heat_infiltration_opaque")
            or self._heat_infiltration_opaque is None
        ):
            floor_surface_area = (
                self.conditioned_floor_projection.sum()
            )  # [Inputs cell E259]
            wall_opaque_surface_area = (
                self.conditioned_facade_areas.sum() - self.window_areas.sum()
            )  # [C259]
            roof_surface_area = (
                self.roof_projections.sum() * self.roof_factor
            )  # [Inputs cell D259]

            floor_surface_transmission = (
                floor_surface_area * self.spec.uvalue_floor
            )  # [Inputs cell E260]
            wall_surface_transmission = (
                wall_opaque_surface_area * self.spec.uvalue_facade
            )  # [C260]
            roof_surface_transmission = (
                roof_surface_area * self.spec.uvalue_roof
            )  # [Inputs cell D260]

            # [Inputs cell C266]
            if self.spec.thermal_bridge_facade_ground:
                floor_transmittance = THERMAL_BREAK_TRANSMITTANCE[
                    THERMAL_BREAKS.Facade_ground
                ]
            else:
                floor_transmittance = 0.0
            if self.spec.thermal_bridge_facade_intermediate:
                other_floors_transmittance = THERMAL_BREAK_TRANSMITTANCE[
                    THERMAL_BREAKS.Facade_intermediate
                ]
            else:
                other_floors_transmittance = 0.0
            if self.spec.thermal_bridge_window:
                window_transmittance = THERMAL_BREAK_TRANSMITTANCE[
                    THERMAL_BREAKS.Windows
                ]
            else:
                window_transmittance = 0.0
            if self.spec.thermal_bridge_facade_roof:
                roof_transmittance = THERMAL_BREAK_TRANSMITTANCE[
                    THERMAL_BREAKS.Facade_roof
                ]
            else:
                roof_transmittance = 0.0

            floor_linear_transmission = (
                self.conditioned_floor_perimeters[FLOORS.Ground] * floor_transmittance
            )
            ...
            for i, floor in enumerate(FLOORS):
                ...
                prev_perimeter = self.conditioned_floor_perimeters[
                    FLOORS.get_by_index(i - 1)
                ]
                if prev_perimeter <= 0:
                    ...
                    other_floors_thermal_bridge_perimeter += my_perimeter * (
                        my_perimeter / prev_perimeter
                    )
            other_floors_linear_transmission = (
                other_floors_thermal_bridge_perimeter * other_floors_transmittance
            )
            ...
            window_perimeter = (self.window_areas.sum() / self.spec.window_height) + (
                self.window_areas.sum() / self.spec.window_height * 2
            ) * window_external_perimeter_rate
            windows_linear_transmission = window_perimeter * window_transmittance

            # [Inputs cells D266, F266]
            wall_linear_transmission = (
                other_floors_linear_transmission + windows_linear_transmission
            )
            ...
            for i, floor in enumerate(FLOORS):
                if floor == FLOORS.get_by_index(-1):
                    ...
                    next_perimeter = self.conditioned_floor_perimeters[
                        FLOORS.get_by_index(i + 1)
                    ]
                    my_perimeter = self.conditioned_floor_perimeters[floor]

...
class HeatingSystemSimulation(EnergyUseSimulation):
    ...
    def demand(self) -> "Series[float]":
        ...
        if "demand" not in self._hours.columns:
            ...
            self._hours["demand"] = self._hours["demand"].clip(
                upper=MAX_HEATING_CAPACITY_LIMIT * self.capacity
            )
    ...
    def capacity(self):
        ...
        )  # [1 is the capacity factor, hardcoded in EY93]
    ...
    def nominal_capacity(self) -> float:
        ...
        if self._nominal_capacity is None:
            self._nominal_capacity = (
                max(self._attr("nominal_capacity"), MIN_HEATING_CAPACITY) * self.number
            )

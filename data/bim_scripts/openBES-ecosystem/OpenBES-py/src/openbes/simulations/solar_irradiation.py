...
class SolarIrradiationSimulation:
    ...
    def ghi(self) -> "Series[float]":
        ...
        if "global_horizontal_irradiance" not in self._hours.columns:
            self._hours["global_horizontal_irradiance"] = list(
                self.epw_data["ghi"].astype(float)
            )
    ...
    def dni(self) -> "Series[float]":
        ...
        if "direct_normal_irradiance" not in self._hours.columns:
            self._hours["direct_normal_irradiance"] = list(
                self.epw_data["dni"].astype(float)
            )
    ...
    def dhi(self) -> "Series[float]":
        ...
        if "diffuse_horizontal_irradiance" not in self._hours.columns:
            self._hours["diffuse_horizontal_irradiance"] = list(
                self.epw_data["dhi"].astype(float)
            )
    ...
    def day_of_year(self):
        ...
        return np.array(
            self._hours.index.get_level_values(self._hours.index.names.index("day"))
        )
    ...
    def hour_offset(self):
        ...
        return (
            np.array(
                self._hours.index.get_level_values(
                    self._hours.index.names.index("hour")
                )
            )
            - 0.5
        )
    ...
    def _hour_angle(self):
        ...
        if self._hour_angle_ is None:
            orbital_position = (
                2 * np.pi * (self.day_of_year - 1 + (self.hour_offset - 12) / 24) / 365
            )
    ...
    def _solar_declination(self):
        ...
        if self._solar_declination_ is None:
            gamma = (
                2 * np.pi / 365 * (self.day_of_year - 1 + (self.hour_offset - 12) / 24)
            )
    ...
    def solar_altitude(self) -> "Series[float]":
        ...
        if "solar_altitude" not in self._hours.columns:
            ...
            sin_solar_altitude = np.cos(latitude) * np.cos(
                self._solar_declination
            ) * np.cos(self._hour_angle) + np.sin(latitude) * np.sin(
                self._solar_declination
            )
    ...
    def cos_zenith(self) -> "Series[float]":
        if "cos_zenith" not in self._hours.columns:
            ...
            self._hours["cos_zenith"] = sin_phi * np.sin(
                self._solar_declination
    ...
    def solar_azimuth(self) -> "Series[float]":
        ...
        if "solar_azimuth_degrees" not in self._hours.columns:
            ...
            solar_azimuth = np.degrees(
                np.atan2(
                    np.sin(self._hour_angle),
                    np.cos(self._hour_angle) * sin_phi
                    - np.tan(self._solar_declination) * cos_phi,
                )
            )
            solar_azimuth += 180.0
            solar_azimuth %= 360.0
    ...
    def get_aoi(self, compass_point: COMPASS_POINTS) -> "Series[float]":
        ...
        if self._aoi is None:
            ...
            for point in list(COMPASS_POINTS):
                azimuth_component = np.radians(
                    self.solar_azimuth - self.surface_azimuths[point]
                )
                self._aoi[point] = np.cos(r90) * self.cos_zenith + np.sin(r90) * np.sin(
                    np.radians(self.solar_zenith)
                ) * np.cos(azimuth_component)
    ...
    def get_aoi_degrees(self, compass_point: COMPASS_POINTS) -> "Series[float]":
        ...
        return np.degrees(
            np.acos(np.minimum(np.maximum(self.get_aoi(compass_point), -1), 1))
        )
    ...
    def dni_extra(self) -> "Series[float]":
        ...
        if "dni_extra" not in self._hours.columns:
            dni_extra = 1367 * (1 + 0.033 * np.cos(2 * np.pi * self.day_of_year / 365))
    ...
    def relative_air_mass(self) -> "Series[float]":
        ...
        if "relative_air_mass" not in self._hours.columns:
            ram = 1 / (
                np.cos(np.radians(self.solar_zenith))
                + 0.50572 * ((96.07995 - self.solar_zenith) ** -1.6364)
            )
    ...
    def clearness(self) -> "Series[float]":
        ...
        if "clearness" not in self._hours.columns:
            self._hours["clearness"] = np.where(
                ...
                (
                    (
                        (self.dhi + self.dni) / self.dhi
                        + 1.041 * np.radians(self.solar_zenith) ** 3
                    )
    ...
    def get_perez_coefficient(self, coefficient: str) -> "Series[float]":
        ...
        if coefficient not in self._hours.columns:
            idx = np.clip(
                np.searchsorted(self.perez_bins["eps_lo"], self.clearness, side="right")
                - 1,
                0,
    ...
    def ground_reflected_component(self) -> "Series[float]":
        ...
        if "ground_reflected_component" not in self._hours.columns:
            self._hours["ground_reflected_component"] = (
                self.ghi * 0.2 * (1 - np.cos(np.radians(90))) / 2
            )
    ...
    def get_solar_irradiation(self, point: COMPASS_POINTS) -> "Series[float]":
        ...
        if point not in self._solar_irradiation.columns:
            self._solar_irradiation[point] = (
                self.get_diffuse_component(point)
                + self.get_beam_component(point)
                + self.ground_reflected_component
            )
